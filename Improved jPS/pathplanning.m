...
Environmental_Set=0;  %判断是否随机生成障碍物，为0时会保留上次生成的障碍物信息

obstacle=GetBoundary(map);%得到边界数据
nObstacle=400;%在地图中随机加入XX个障碍物
...
%初始化closelist
close=[];
open1=[map.goal(1),map.goal(2),0,0,0,h(map,map.start,map.goal),0,0];
close1=[];
[open,close,open1,close1]=AStar(open1,close1,open,close,obstacle,map);
%画出路径
%
...
for pp1=1:length(close1(:,1))
    x1 = close1(pp1,1);
    y1 = close1(pp1,2);
    A1(pp1,1)=x1;
    A1(pp1,2)=y1;%这部分是将路径坐标拿出来另外存放
end
plot( A1(:,1), A1(:,2),'b','linewidth',4)   %绘制路线
...
k1=0;%用于存放路径长度的变量
for i=1:length(close1(:,1))-1
    b=10*sqrt(((close1(i+1,1)-close1(i,1))^2)+((close1(i+1,2)-close1(i,2))^2));%简单的两点间距离公式
    k1=k1+b;%路径长度值的逐个累加
end
disp(['路径长度为',num2str(k+k1)]);